...
-- MAGIC ## Overview
-- MAGIC
-- MAGIC In this demo, we will see how to create compute resources that can access a Unity Catalog metastore. Prior to Unity Catalog, a full data governance solution required careful configuration of workspace settings, access control lists, and cluster settings and policies. It was a co-operative system that, if not properly set up, could allow users to bypass access control altogether.
-- MAGIC
...
-- MAGIC 1. Now let’s choose a **Databricks runtime version**. Let’s choose the most recent version which is *11.1* at the time of this recording.
...
-- MAGIC     * *User isolation* clusters can be shared by multiple users, but overall funcionality is reduced to promote a secure, isolated environment. Only Python and SQL are allowed, and some advanced cluster features such as library installation, init scripts and the DBFS Fuse mounts are also unavailable.
...
-- MAGIC 1. Create another new cell with the query `SELECT * FROM main.information_schema.tables` and run it.
...
-- MAGIC 1. From here we can also create objects or manage permissions, however we will reservee these tasks for a later lab.
...
-- MAGIC
-- MAGIC Let's repeat the queries we did earlier in the Data Science and Engineering worksapce, this time creating new DBSQL queries and running them using the SQL warehouse we just created. As a recap, these queries were `SHOW GRANTS ON SCHEMA main.default` and `SELECT * FROM main.information_schema.tables`.
